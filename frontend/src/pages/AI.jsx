...
const AI = () => {
  const categories = useStore((state) => state.categories);
   const thisCat = categories.find((c)=>c.name === "AI & Automation");
   const tasks = thisCat.tasks;
  return (
    <section>
        {/* Head div */}
        <div className='flex flex-col justify-center py-12 rounded-lg bg-gradient-to-r from-violet-700 to-violet-950'>
            <h1 className='font-bold text-center text-white sm:text-4xl'>AI Integration</h1>
            <h3 className='mt-4 text-center text-white sm:text-2xl'>You think it. A programmer develops it.</h3>
            <button className='px-8 py-2 mx-auto mt-8 font-semibold text-white uppercase border-2 border-white hover:bg-violet-600'>Contact Us</button>
        </div>
